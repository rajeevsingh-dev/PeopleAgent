...
class PeopleAgent:
    ...
    async def _process_query_core_old(self, user_query):
        """
        Main method:
          1) Query analysis
          2) Data fetching
    ...
    async def _process_query_core(self, user_query):
        """
        Main method:
          1) Data fetching in parallel from all sources
        ...
        # Call all data sources in parallel
        profile_task = asyncio.create_task(self.get_user_profile())
        manager_task = asyncio.create_task(self.get_manager_info())
        reports_task = asyncio.create_task(self.get_direct_reports())
        devices_task = asyncio.create_task(self.get_devices())
        colleagues_task = asyncio.create_task(self.get_colleagues())
        documents_task = asyncio.create_task(self.get_documents())
        all_users_task = asyncio.create_task(self.get_all_users())

        (profile_data, manager_data, reports_data, devices_data,
         colleagues_data, documents_data, all_users_data) = await asyncio.gather(
            profile_task, manager_task, reports_task, devices_task, colleagues_task, documents_task, all_users_task
        )
        
        # Combine all source results into one context variable
        context = {
            "profile": self.format_data("profile", profile_data),
            "manager": self.format_data("manager", manager_data),
            "reports": self.format_data("reports", reports_data),
            "devices": self.format_data("devices", devices_data),
            "colleagues": self.format_data("colleagues", colleagues_data),
            "documents": self.format_data("documents", documents_data),
            "all_users": self.format_data("all_users", all_users_data)
        }
        
        # Log the gathered context for debugging
        self.logger.debug(f"Parallel API calls completed. Context: {context}")
        
       
        # Generate response with fetched context
        response = self.generate_response(user_query, context)
